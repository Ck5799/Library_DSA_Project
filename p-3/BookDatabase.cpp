...
{
  std::ifstream fin("Sample_Book_Database.dat", std::ios::binary );
...
{
    return find(isbn, bookDatabase.begin());
}

Book* BookDatabase::find(const std::string& isbn, const std::vector<Book>::iterator& place)
{
    if (place == bookDatabase.end()) return nullptr;

    if (isbn == place->isbn())
    {
        Book* book_ptr = new Book(*place);
        return book_ptr;
    }

    return find(isbn, place + 1);
}
