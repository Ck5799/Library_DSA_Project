...
void BookList::insert( const Book & book, std::size_t offsetFromTop )       // insert new book at offsetFromTop, which places it before the current book at offsetFromTop
{
  ...
  ///////////////////////// TO-DO (3) //////////////////////////////
    ...
    /// implement it again.
    if (find(book) != _books_vector.size())
      return;
  ...
  { /**********  Part 1 - Insert into array  ***********************/
    ///////////////////////// TO-DO (4) //////////////////////////////
     ...
     if(_books_array_size >= _books_array.size()){
      throw CapacityExceeded_Ex("Capacity Was Exceeded" exception_location);
     }
     ...
     std::move_backward(it,std::next(_books_array.begin(),size()),std::next(_books_array.begin()+size()));
     *it = book;
     _books_array_size++;
     }
    /////////////////////// END-TO-DO (4) ////////////////////////////
...
void BookList::remove( std::size_t offsetFromTop )
{
  ...
  { /**********  Part 1 - Remove from array  ***********************/
    ///////////////////////// TO-DO (8) //////////////////////////////
      ...
      for(int i= offsetFromTop;i<_books_array_size-1; i++){
  ...
  { /**********  Part 2 - Remove from vector  **********************/
    ///////////////////////// TO-DO (9) //////////////////////////////
     ...
     for(int i=0;i<offsetFromTop;i++)it++;
...
BookList & BookList::operator+=( const BookList & rhs )
{
  ///////////////////////// TO-DO (14) //////////////////////////////
    ...
    /// container you picked inserting its books to the bottom of this book list. Use BookList::insert() to insert at the bottom.
         for(auto && book : rhs._books_sl_list){
                insert(book,Position::BOTTOM);
         }
  /////////////////////// END-TO-DO (14) ////////////////////////////
...
std::weak_ordering BookList::operator<=>( BookList const & rhs ) const
{
  ...
  ///////////////////////// TO-DO (15) //////////////////////////////
    ...
    int Ext;
    if (rhs.size()>size())Ext=rhs.size();
    else Ext= size();
    for (int i=0;i<Ext;i++){
      if(_books_vector[i] != rhs._books_vector[i]){
        return _books_vector[i] <=> rhs._books_vector[i];
      }
    }
