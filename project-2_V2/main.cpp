...
int main()
{
 ...
 BookList myBooklist ={{"Big Bang Theory" },{"Two states"},{"Meluha"},{"Omnicient Reader's Viewpoint"}};

     std::cout<<"My Book list is : "<<myBooklist<<std::endl;

     myBooklist.remove({"Big Bang Theory"});
     myBooklist.insert({"Eminence in the Shadows"});
     myBooklist.moveToTop({"Meluha"});

     std::cout<<"The list is : "<<myBooklist<<std::endl;

     BookList otherBooklist= {{"Oliver Twist", "Charles Dickens" },{"A Tale of Two Cities"            },{"Ulysses"                         },{"The Color Purple"                }, {"Hunger Games", "Suzanne Collins" },{"Charlie & Choc Factory"}};
     BookList totalBooklist = myBooklist +=otherBooklist;
     std::cout<<"The Total List is :"<<totalBooklist<<std::endl;
